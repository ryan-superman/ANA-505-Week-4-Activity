...
ThirdColor <- as.data.frame(SecondColor)
select(ThirdColor,Hair,Sex)

#Let's name the dataset with just the two columns, Hair and Sex
#TASK: Write the function to save the two columns as a new dataset
#and give it a name
SexHair <- select(ThirdColor,Hair,Sex)
dim(SexHair)
...
GenderHair <- rename(SexHair, Gender = Sex)
...
total=mutate(ThirdColor, total=cumsum(Freq))
